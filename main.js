...
var ImgbbValidator = class {
  ...
  static async validateApiKey(apiKey) {
    ...
    try {
      ...
      if (response.status === 200 && ((_a2 = response.json) == null ? void 0 : _a2.success)) {
        return {
          valid: true
        };
...
var MTJSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Output format").setDesc("Choose the target markup format for conversion.").addDropdown((dropdown) => {
      dropdown.addOption("jira", "Jira").addOption("confluence", "Confluence").setValue(this.plugin.settings.outputFormat).onChange(async (value) => {
        this.plugin.settings.outputFormat = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Render metadata").setDesc("Transforms the metadata at the top of an Obsidian note, enclosed by ---, into a Jira-compatible format, resulting in a list of key-value pairs labeled as 'Metadata'.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.renderMetadata).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Output formatting").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Image embed style").setDesc(
      "How image markup is emitted. 'Thumbnail' produces !path|thumbnail!, 'Alt text' produces !path|alt=...!, 'Plain' produces !path!."
    ).addDropdown((dropdown) => {
      dropdown.addOption("thumbnail", "Thumbnail (|thumbnail)").addOption("alt", "Alt text (|alt=...)").addOption("plain", "Plain (no args)").setValue(this.plugin.settings.imageEmbedStyle).onChange(async (value) => {
        this.plugin.settings.imageEmbedStyle = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Wrap local images in warning panel").setDesc(
      "When on, local images that must be uploaded manually are preceded by a yellow {panel} warning. Off by default \u2014 the converted image list in the preview modal makes manual uploads easier to track."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.imageWarningPanel).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Code block style").setDesc(
      "How fenced code blocks are emitted. '{code}' adds syntax highlighting (e.g. {code:js}); '{noformat}' is plain preformatted text for older Jira instances that don't render {code} blocks."
    ).addDropdown((dropdown) => {
      dropdown.addOption("code", "{code} (syntax highlighted)").addOption("noformat", "{noformat} (plain, older Jira)").setValue(this.plugin.settings.codeBlockStyle).onChange(async (value) => {
        this.plugin.settings.codeBlockStyle = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Clipboard detection").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Auto-detect Jira markup on paste").setDesc("When pasting, detect if the clipboard contains Jira/Confluence markup and offer to convert it to Markdown.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoDetectJiraPaste).onChange(async (value) => {
        this.plugin.settings.autoDetectJiraPaste = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Task lists").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Enable task list visualization").setDesc(
      "Convert Markdown task list checkboxes (e.g., [ ], [x], [>]) to Jira emoticons in the output. This makes task status visible in Jira comments and descriptions."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.taskListVisualization.enabled).onChange(async (value) => {
    ...
    if (this.plugin.settings.taskListVisualization.enabled) {
      ...
      new import_obsidian2.Setting(containerEl).setName("Custom task state mappings").setDesc("Add your own custom checkbox states (e.g., [!], [?], [*])");
      const customMappings = Object.entries(this.plugin.settings.taskListVisualization.mapping).filter(([key]) => !taskStates.some((ts) => ts.key === key));
      for (const [key, value] of customMappings) {
        new import_obsidian2.Setting(containerEl).setName("Custom mapping").addText((text2) => {
          text2.setPlaceholder("[?]").setValue(key).onChange(async (newKey) => {
            if (newKey !== key) {
      ...
      new import_obsidian2.Setting(containerEl).addButton((button) => {
        button.setButtonText("Add custom mapping").setIcon("plus").onClick(async () => {
          let newKey = "[?]";
          let counter = 1;
    ...
    new import_obsidian2.Setting(containerEl).setName("Mentions & issue links").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Convert @mentions").setDesc("Convert @username mentions to Jira user mentions [~username].").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.convertMentions).onChange(async (value) => {
        this.plugin.settings.convertMentions = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Enable Jira issue linking").setDesc("Automatically convert issue keys (e.g., PROJ-123) to Jira links.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.jiraIssueLink.enabled).onChange(async (value) => {
    ...
    if (this.plugin.settings.jiraIssueLink.enabled) {
      new import_obsidian2.Setting(containerEl).setName("Project keys").setDesc("Comma-separated list of Jira project keys (e.g., PROJ,DEV,OPS).").addText(
        (text2) => text2.setPlaceholder("PROJ,DEV,OPS").setValue(this.plugin.settings.jiraIssueLink.projectKeys).onChange(async (value) => {
          this.plugin.settings.jiraIssueLink.projectKeys = value;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian2.Setting(containerEl).setName("Jira base URL").setDesc("Your Jira instance URL (e.g., https://your-company.atlassian.net).").addText(
        (text2) => text2.setPlaceholder("https://your-company.atlassian.net").setValue(this.plugin.settings.jiraIssueLink.baseUrl).onChange(async (value) => {
          this.plugin.settings.jiraIssueLink.baseUrl = value;
          await this.plugin.saveSettings();
        })
      );
    }
    new import_obsidian2.Setting(containerEl).setName("Mermaid diagrams").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Mermaid diagram handling").setDesc("Choose how to handle Mermaid diagram code blocks during conversion.").addDropdown((dropdown) => {
      dropdown.addOption("code-block", "Keep as code block").addOption("plantuml", "Convert to PlantUML (basic)").addOption("warning", "Show warning message").setValue(this.plugin.settings.mermaidHandling).onChange(async (value) => {
        this.plugin.settings.mermaidHandling = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Callouts").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Add callout configuration").setDesc(
      ...
      button.setIcon("plus").onClick(async () => {
        if (settings.temp.createCalloutConfiguration.length == 0) {
          return new import_obsidian2.Notice(
            "First select a callout type to add!"
          );
        }
        if (settings.calloutConfigurations.some(
          (ccfg) => ccfg.identifier == settings.temp.createCalloutConfiguration
        )) {
          return new import_obsidian2.Notice(
      ...
      button.setIcon(this.plugin.settings.showCalloutConfiguration ? "up-chevron-glyph" : "down-chevron-glyph").setTooltip(this.plugin.settings.showCalloutConfiguration ? "Hide callout configuration" : "Show callout configuration").onClick(async () => {
    ...
    if (this.plugin.settings.showCalloutConfiguration) {
      for (const [i, val] of settings.calloutConfigurations.entries()) {
        ...
        new import_obsidian2.Setting(containerEl).setName(`Choose title background color:`).addColorPicker((colorPicker) => {
          colorPicker.setValue(
        ...
        new import_obsidian2.Setting(containerEl).setName(`Choose content text color:`).addColorPicker((colorPicker) => {
          colorPicker.setValue(
        ...
        new import_obsidian2.Setting(containerEl).setName(`Choose content background color:`).addColorPicker((colorPicker) => {
          colorPicker.setValue(
        ...
        new import_obsidian2.Setting(containerEl).setName(`Choose content border color:`).addColorPicker((colorPicker) => {
          colorPicker.setValue(
        ...
        new import_obsidian2.Setting(containerEl).setName("Remove configuration").addButton((button) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Image handling").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Image upload method").setDesc(
      "Choose how to handle local images when converting to Jira markup. Manual: show warning to drag & drop manually. ImgBB: auto-upload to ImgBB (free API key required)."
    ).addDropdown((dropdown) => {
      dropdown.addOption("manual", "Manual (drag & drop)").addOption("imgbb", "Auto-upload to ImgBB").setValue(this.plugin.settings.imageUpload.method).onChange(async (value) => {
    ...
    if (this.plugin.settings.imageUpload.method === "imgbb") {
      ...
      new import_obsidian2.Setting(containerEl).setName("ImgBB API Key").setDesc(
        "Get your free API key by creating an account at https://imgbb.com and visiting https://api.imgbb.com. ImgBB supports images up to 32 MB with unlimited uploads."
      ).addText(
        (text2) => text2.setPlaceholder("Enter API Key").setValue(this.plugin.settings.imageUpload.imgbb.apiKey).onChange(async (value) => {
        ...
        button.setButtonText("Test API Key").onClick(async () => {
          ...
          try {
            ...
            if (result.valid) {
              new import_obsidian2.Notice(MESSAGES.SUCCESS.API_KEY_VALID);
            } else {
              new import_obsidian2.Notice(`API key validation failed: ${result.error}`);
            }
          } catch (error2) {
            new import_obsidian2.Notice(`Validation error: ${error2 instanceof Error ? error2.message : String(error2)}`);
          } finally {
...
// src/utils/imageMarkup.ts
function renderImageMarkup(src, alt, style) {
  switch (style) {
    case "thumbnail":
...
function renderWarningPanel(src) {
...
function wikiLinks(md, options) {
  var _a2, _b, _c;
  const settings = (_a2 = options == null ? void 0 : options.translator) == null ? void 0 : _a2.plugin.settings;
  const embedStyle = (_b = settings == null ? void 0 : settings.imageEmbedStyle) != null ? _b : "alt";
  const warningPanel = (_c = settings == null ? void 0 : settings.imageWarningPanel) != null ? _c : false;
  md.inline.ruler.before("link", "wikilink", (state, silent) => {
    const max = state.posMax;
    const start = state.pos;
    const isImage = state.src.charCodeAt(start) === 33;
    const linkStart = isImage ? start + 1 : start;
    if (state.src.charCodeAt(linkStart) !== 91 || state.src.charCodeAt(linkStart + 1) !== 91) {
      return false;
    }
    let pos = linkStart + 2;
    while (pos < max) {
  ...
  md.renderer.rules.wiki_image = (tokens, idx) => {
    ...
    const alt = ((_a3 = parts[1]) == null ? void 0 : _a3.trim()) || filename;
    if (options == null ? void 0 : options.translator) {
      return options.translator.registerImage(filename, alt);
    }
    const inline2 = renderImageMarkup(filename, alt, embedStyle);
    if (warningPanel) {
      return `{panel:borderColor=#ffecb5|bgColor=#fff3cd}
...
function basics(md, options) {
  ...
  md.renderer.rules.link_open = (tokens, idx, options2, env) => {
    env.currentHref = tokens[idx].attrGet("href");
    return `[`;
  };
  md.renderer.rules.link_close = (tokens, idx, options2, env) => {
  ...
  md.renderer.rules.image = (tokens, idx) => {
    ...
    if (Validator.isUrlOrBase64(src)) {
      if (src != null && (options == null ? void 0 : options.translator)) {
        options.translator.registerLocalImage(src, alt, false);
      }
      return renderImageMarkup(src, alt, embedStyle);
    }
    if ((options == null ? void 0 : options.translator) && src != null) {
      return options.translator.registerImage(src, alt);
    }
    const inline2 = renderImageMarkup(src, alt, embedStyle);
    if (warningPanel) {
      return `{panel:borderColor=#ffecb5|bgColor=#fff3cd}
{color:#664d03}+*Warning:*+ The following file must be transferred manually via drag & drop: *${src}*{color}
{panel}
...
// src/rules/mermaid.ts
function mermaid(md, handling, codeBlockStyle = "code") {
...
function convertToPlantUML(mermaidCode, codeBlockStyle = "code") {
  const lines = mermaidCode.split("\n");
  const plantUmlLines = ["@startuml"];
  let diagramType = "";
...
function convertFlowchartLine(line) {
...
function convertSequenceLine(line) {
  ...
  if (line.startsWith("Note ")) {
    return line.replace("Note over ", "note over ").replace("Note right of ", "note right of ").replace("Note left of ", "note left of ");
  }
  return `' ${line}`;
}

// src/rules/mentions.ts
function mentions(md) {
...
function isValidUsernameStartChar(code2) {
  return code2 >= 97 && code2 <= 122 || code2 >= 65 && code2 <= 90;
}
function isValidUsernameChar(code2) {
  return (
    // a-z
    code2 >= 97 && code2 <= 122 || // A-Z
    code2 >= 65 && code2 <= 90 || // 0-9
    code2 >= 48 && code2 <= 57 || // . _ -
    code2 === 46 || code2 === 95 || code2 === 45
  );
}
function isValidPrecedingChar(code2) {
  if (code2 === 32 || code2 === 9 || code2 === 10 || code2 === 13) {
    return true;
  }
  if (code2 === 40 || code2 === 91 || code2 === 123) {
    return true;
  }
  if (code2 === 44 || code2 === 59 || code2 === 58 || code2 === 33 || code2 === 63 || code2 === 46 || code2 === 47) {
    return true;
  }
  return false;
}

// src/rules/issueLinks.ts
function issueLinks(md, options) {
  if (!options.projectKeys || options.projectKeys.length === 0) {
    return;
  }
  const keysPattern = options.projectKeys.map((key) => key.trim().toUpperCase()).filter((key) => key.length > 0).join("|");
  if (!keysPattern) {
    return;
  }
  ...
  md.renderer.rules.text = (tokens, idx, opts, env, self) => {
    ...
    const result = content.replace(issuePattern, (match2, project, number) => {
  ...
  md.core.ruler.push("issue_links", (state) => {
    for (const blockToken of state.tokens) {
      ...
      for (const token of blockToken.children) {
        ...
        let match2;
        const content = token.content;
        while ((match2 = issuePattern.exec(content)) !== null) {
          if (match2.index > lastIndex) {
            const textToken = new state.Token("text", "", 0);
            textToken.content = content.slice(lastIndex, match2.index);
            newChildren.push(textToken);
          }
          const issueToken = new state.Token("issue_link", "", 0);
          issueToken.content = match2[0];
          issueToken.meta = {
            project: match2[1],
            number: match2[2],
  ...
  md.renderer.rules.issue_link = (tokens, idx) => {
    ...
    const baseUrl = ((_b = (_a2 = token.meta) == null ? void 0 : _a2.baseUrl) == null ? void 0 : _b.replace(/\/$/, "")) || "";
    if (baseUrl) {
      return `[${issueKey}|${baseUrl}/browse/${issueKey}]`;
    }
    return `[${issueKey}]`;
...
// src/services/ImageHandler.ts
var import_obsidian4 = require("obsidian");

// src/services/ImgbbUploader.ts
var import_obsidian3 = require("obsidian");
var ImgbbUploader = class {
  constructor(apiKey) {
    this.apiKey = apiKey;
  }
  async uploadImage(imageData, filename) {
    ...
    try {
      ...
      if (response.status === 200 && ((_b = (_a2 = response.json) == null ? void 0 : _a2.data) == null ? void 0 : _b.url)) {
        return {
          success: true,
      ...
      console.error(`[ImgbbUploader] Upload error:`, error2);
      return {
        success: false,
        error: `ImgBB upload error: ${error2 instanceof Error ? error2.message : String(error2)}`
      };
    }
  }
...
var ImageHandler = class {
  ...
  async uploadToImgbb(src, alt) {
    try {
      ...
      if (result.success && result.url) {
        new import_obsidian4.Notice(`Image uploaded to ImgBB: ${file.name}`);
        return {
          jiraMarkup: renderImageMarkup(result.url, alt, this.opts.embedStyle),
          success: true
        };
      } else {
        new import_obsidian4.Notice(`Failed to upload ${file.name}: ${result.error}`);
        return {
          jiraMarkup: this.manualHandling(src, alt).jiraMarkup,
          success: false,
          error: result.error
  ...
  manualHandling(src, alt) {
    const inline2 = renderImageMarkup(src, alt, this.opts.embedStyle);
    const markup = this.opts.warningPanel ? `${renderWarningPanel(src)}
${inline2}` : inline2;
...
var Translator = class {
   ...
   */
  convertMarkdownToJiraSync(markdown) {
    this.collectedImages = [];
...
`;
   ...
   */
  async convertMarkdownToJira(markdown) {
    if (this.plugin.settings.imageUpload.method === "manual" && !this.containsProcessableImages(markdown)) {
...
`;
   ...
   */
  createMarkdownIt() {
    const md = new lib_default().use(basics, { translator: this }).use(wikiLinks, { translator: this }).use(taskLists, this.plugin.settings.taskListVisualization).use(callouts, this.plugin.settings.calloutConfigurations);
    ...
    if (this.plugin.settings.jiraIssueLink.enabled && this.plugin.settings.jiraIssueLink.projectKeys) {
      const projectKeys = this.plugin.settings.jiraIssueLink.projectKeys.split(",").map((k) => k.trim()).filter((k) => k.length > 0);
...
// src/rules/confluenceCallouts.ts
var CONFLUENCE_MACRO_MAP = {
  // Info types
  "NOTE": "info",
  "INFO": "info",
  "ABSTRACT": "info",
...
function confluenceCallouts(md, calloutConfigs) {
  md.block.ruler.before("blockquote", "confluence_callout", (state, startLine, endLine, silent) => {
    ...
    const calloutTitle = ((_a2 = calloutMatch[2]) == null ? void 0 : _a2.trim()) || calloutType;
    const contentLines = [];
    let nextLine = startLine + 1;
    while (nextLine < endLine) {
      const pos = state.bMarks[nextLine] + state.tShift[nextLine];
...
// src/core/ConfluenceTranslator.ts
var ConfluenceTranslator = class {
  constructor(plugin) {
    this.plugin = plugin;
    this.imageHandler = new ImageHandler(plugin.app, plugin.settings.imageUpload, {
      embedStyle: plugin.settings.imageEmbedStyle,
...
// src/core/ReverseTranslator.ts
var JiraTokenizer = class {
  tokenize(input) {
    const tokens = [];
  ...
  parseCodeBlock(lines, startIndex) {
    ...
    const lang = ((_a2 = openMatch[1]) == null ? void 0 : _a2.slice(1)) || "";
    const contentLines = [];
    let i = startIndex + 1;
  ...
  parsePanelBlock(lines, startIndex) {
  ...
  parseNoformatBlock(lines, startIndex) {
  ...
  parseConfluenceMacro(lines, startIndex, macroType) {
  ...
  parseExpandMacro(lines, startIndex) {
...
var ReverseTranslator = class {
  ...
  renderToken(token) {
    ...
    switch (token.type) {
      case "heading":
        return "#".repeat(((_a2 = token.meta) == null ? void 0 : _a2.level) || 1) + " " + token.content + "\n\n";
      case "code_block": {
        const lang = ((_b = token.meta) == null ? void 0 : _b.lang) || "";
        return "```" + lang + "\n" + token.content + "\n```\n\n";
      }
      case "noformat":
        return "```\n" + token.content + "\n```\n\n";
      ...
      case "list_item": {
        const indent = "  ".repeat((((_c = token.meta) == null ? void 0 : _c.level) || 1) - 1);
        const marker = ((_d = token.meta) == null ? void 0 : _d.ordered) ? "1." : "-";
        return indent + marker + " " + token.content + "\n";
      }
      case "panel": {
        const panelTitle = ((_e = token.meta) == null ? void 0 : _e.title) || "NOTE";
        const panelContent = token.content.replace(/\{color:[^}]+\}/g, "").replace(/\{color\}/g, "").trim();
        return `> [!NOTE] ${panelTitle}
` + panelContent.split("\n").map((line) => "> " + line).join("\n") + "\n\n";
      }
      case "confluence_macro": {
        const macroType = ((_f = token.meta) == null ? void 0 : _f.macroType) || "info";
        const macroTitle = ((_g = token.meta) == null ? void 0 : _g.title) || "";
        const macroContent = token.content.replace(/\{color:[^}]+\}/g, "").replace(/\{color\}/g, "").trim();
...
` + macroContent.split("\n").map((line) => "> " + line).join("\n") + "\n\n";
      ...
      case "expand": {
        const expandTitle = ((_h = token.meta) == null ? void 0 : _h.title) || "Details";
...
var UpdateModal = class extends import_obsidian5.Modal {
  ...
  async onOpen() {
    ...
    try {
      ...
      console.error("[UpdateModal] Failed to fetch release notes:", error2);
    }
  ...
  async fetchReleaseNotes() {
    ...
    try {
      ...
      if (response.status === 200 && ((_a2 = response.json) == null ? void 0 : _a2.body)) {
      ...
      console.error("[UpdateModal] Error fetching release notes:", error2);
      this.releaseNotes = null;
    }
  }
};
...
var PreviewModal = class extends import_obsidian6.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian6.ButtonComponent(buttonContainer).setButtonText("Close").onClick(() => {
      this.close();
    });
  ...
  renderImageList(container, images) {
    ...
    section.createEl("p", {
      text: "Jira can't reference your vault's local files \u2014 drag the images into your Jira issue.",
      cls: "mtj-image-list-hint"
    });
    if (import_obsidian6.Platform.isDesktopApp && this.app.vault.adapter instanceof import_obsidian6.FileSystemAdapter) {
      const actions = section.createDiv({ cls: "mtj-image-list-actions" });
      new import_obsidian6.ButtonComponent(actions).setButtonText(`Open all ${images.length} in folder`).setCta().setTooltip(
    ...
    const list2 = section.createEl("ul", { cls: "mtj-image-list-items" });
    for (const img of images) {
      const item = list2.createEl("li", { cls: "mtj-image-list-item" });
      item.createSpan({ text: img.src, cls: "mtj-image-list-path" });
      const btnRow = item.createDiv({ cls: "mtj-image-list-buttons" });
      new import_obsidian6.ButtonComponent(btnRow).setButtonText("Reveal").setTooltip("Reveal file in your OS file manager").onClick(() => this.revealImage(img.src));
      new import_obsidian6.ButtonComponent(btnRow).setButtonText("Copy path").setTooltip("Copy the file path to clipboard").onClick(async () => {
        await navigator.clipboard.writeText(img.src);
        new import_obsidian6.Notice(`Copied: ${img.src}`);
      });
    }
  }
  revealImage(src) {
    ...
    if (file && file instanceof import_obsidian6.TFile) {
      ...
      leaf.openFile(file).catch(() => {
        new import_obsidian6.Notice(`Could not open ${src}`);
      });
      return;
    }
    new import_obsidian6.Notice(`File not found in vault: ${src}`);
  }
  /**
   ...
   */
  async openAllInFolder(images) {
    const adapter = this.app.vault.adapter;
    if (!(adapter instanceof import_obsidian6.FileSystemAdapter)) {
      new import_obsidian6.Notice("Opening a folder is only supported on the desktop app.");
      return;
    }
    const fs = require("fs");
    const os = require("os");
    ...
    try {
      ...
      new import_obsidian6.Notice(`Could not prepare temp folder: ${err.message}`);
      return;
    }
    ...
    for (const img of images) {
      ...
      if (usedNames.has(name)) {
        ...
        const base2 = ext ? name.slice(0, -ext.length) : name;
        let n = 2;
        while (usedNames.has(`${base2}-${n}${ext}`)) n++;
        name = `${base2}-${n}${ext}`;
      }
      usedNames.add(name);
      try {
        ...
        missing.push(`${img.src} (${err.message})`);
      }
    }
    if (copied === 0) {
      new import_obsidian6.Notice("No local images could be staged \u2014 none resolved to vault files.");
      return;
    }
    const openErr = await shell.openPath(dir);
    if (openErr) {
      new import_obsidian6.Notice(`Staged ${copied} image(s) at ${dir}, but could not open it: ${openErr}`);
      return;
    }
  ...
  highlightSyntax(container, markup) {
  ...
  tokenizeLine(line) {
    ...
    if (listMatch) {
      ...
      this.tokenizeInline(listMatch[2], segments2);
      return segments2;
    }
    const segments = [];
  ...
  tokenizeInline(text2, segments) {
    ...
    while (remaining.length > 0) {
      ...
      const link2 = remaining.match(/^\[([^\]|]+)(\|)([^\]]+)\]/);
      if (link2) {
        segments.push({ text: "[", cls: "mtj-syntax-accent" });
        segments.push({ text: link2[1], cls: "mtj-syntax-link-text" });
        segments.push({ text: link2[2], cls: "mtj-syntax-accent" });
        segments.push({ text: link2[3], cls: "mtj-syntax-link-url" });
        segments.push({ text: "]", cls: "mtj-syntax-accent" });
        remaining = remaining.slice(link2[0].length);
        continue;
      }
...
var ClipboardDetector = class {
   ...
   */
  static isLikelyConfluenceMarkup(text2) {
    if (!text2 || text2.trim().length === 0) {
      return false;
    }
   ...
   */
  static detectMarkupType(text2) {
    if (this.isLikelyConfluenceMarkup(text2)) {
      return "confluence";
    }
    if (this.isLikelyJiraMarkup(text2)) {
      return "jira";
    }
    for (const pattern of this.MARKDOWN_PATTERNS) {
      if (pattern.test(text2)) {
...
ClipboardDetector.JIRA_STRONG_PATTERNS = [
  /^h[1-6]\.\s/m,
  // Headings: h1. h2. etc.
  /\{code(:[^}]+)?\}[\s\S]*?\{code\}/,
  // Code blocks
  ...
  // Jira emoticon (/)
  /\(\*[rgby]\)/
  // Jira colored stars (*r), (*g), etc.
];
// Weak Jira patterns (need multiple to be confident)
ClipboardDetector.JIRA_WEAK_PATTERNS = [
  /\[[^\]|]+\|[^\]]+\]/,
  // Links [text|url]
  /!\S+\|[^!]*!/,
  // Images with params !url|param!
  /^\*\s+\S/m,
  // Bullet list starting with * (followed by content)
  /^#\s+\S/m,
  // Numbered list starting with # (followed by content)
  /^-{4,}$/m
  // Horizontal rule ----
];
// Confluence-specific patterns (macros)
ClipboardDetector.CONFLUENCE_PATTERNS = [
  /\{info[^}]*\}[\s\S]*?\{info\}/,
  // Info macro
  /\{warning[^}]*\}[\s\S]*?\{warning\}/,
  // Warning macro
...
ClipboardDetector.MARKDOWN_PATTERNS = [
  /^#{1,6}\s+\S/m,
  // Markdown headings # ## ###
  /```[\s\S]*?```/,
...
// src/modals/ConversionOfferModal.ts
var import_obsidian7 = require("obsidian");
var ConversionOfferModal = class extends import_obsidian7.Modal {
  constructor(app, clipboardContent, markupType, onDecision) {
    super(app);
  ...
  onOpen() {
    ...
    const previewText = this.clipboardContent.length > 200 ? this.clipboardContent.substring(0, 200) + "..." : this.clipboardContent;
    const previewContainer = contentEl.createDiv({ cls: "mtj-conversion-preview" });
    previewContainer.createEl("code", { text: previewText });
    const buttonContainer = contentEl.createDiv({ cls: "mtj-modal-buttons" });
    new import_obsidian7.ButtonComponent(buttonContainer).setButtonText("Keep original").onClick(() => {
      this.close();
      this.onDecision(false);
    });
    new import_obsidian7.ButtonComponent(buttonContainer).setButtonText("Convert to Markdown").setCta().onClick(() => {
      this.close();
...
var MTJPlugin = class extends import_obsidian8.Plugin {
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("editor-paste", (evt, editor) => {
        ...
        const clipboardText = (_a2 = evt.clipboardData) == null ? void 0 : _a2.getData("text/plain");
  ...
  async loadSettings() {
    ...
    if (loadedData) {
      ...
      const oldImageUpload = loadedData.imageUpload;
      if (oldImageUpload && !oldImageUpload.method && (hasOwn(oldImageUpload, "enabled") || hasOwn(oldImageUpload, "host") || hasOwn(oldImageUpload, "username"))) {
      ...
      if (loadedData.imageEnableUploadToHost !== void 0) {
        delete loadedData.imageEnableUploadToHost;
        needsMigration = true;
      }
      const tlv = loadedData.taskListVisualization;
      if (tlv && tlv.enabled === false && Object.keys(tlv.mapping || {}).length === 0) {
